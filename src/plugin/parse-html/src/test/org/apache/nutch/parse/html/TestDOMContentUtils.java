...
public class TestDOMContentUtils {
  ...
  static {
    answerOutlinks = new Outlink[][] {
        { new Outlink("http://www.nutch.org", "anchor"), }, // 1
        ...
        { new Outlink("http://www.nutch.org/index.html", "whitespace test"), }, // 7
        {}, // 8
        { new Outlink("http://www.nutch.org/dummy.jsp", "test2"), }, // 9
        {}, // 10
  ...
  public void testGetText() {
    ...
    for (int i = 0; i < testPages.length; i++) {
      String text = utils.getText(testDOMs[i]);
  ...
  public void testGetTitle() {
    ...
    for (int i = 0; i < testPages.length; i++) {
      String title = utils.getTitle(testDOMs[i]);
  ...
  public void testGetOutlinks() {
    ...
    for (int i = 0; i < testPages.length; i++) {
      ...
      utils.setConf(conf);
      ArrayList<Outlink> outlinks = utils.getOutlinks(testBaseHrefURLs[i], testDOMs[i]);
      compareOutlinks(Lists.newArrayList(answerOutlinks[i]), outlinks, i);
    }
  }
  
  private static String outlinksString(ArrayList<Outlink> o) {
    return o.stream().map(Outlink::toString).collect(Collectors.joining("\n"));
  }

  private static final void compareOutlinks(ArrayList<Outlink> expected, ArrayList<Outlink> actual, int pageIndex) {
    assertEquals("Page : p" + (pageIndex + 1) + "\tExpected : [" + outlinksString(expected) + "]\t Actual : [" + outlinksString(actual) + "]\t",
        expected.size(), actual.size());

    for (int i = 0; i < expected.size(); i++) {
      if (!expected.get(i).equals(actual.get(i))) {
        assertTrue(
            "got wrong outlinks at position " + i + "\n" + "answer: " + "\n" + "'" + expected.get(i).getToUrl()
                + "', anchor: '" + expected.get(i).getAnchor() + "'" +
                "\n" + "got: " + "\n" + "'" + actual.get(i).getToUrl() + "', anchor: '" + actual.get(i).getAnchor() + "'", false);
      }
    }
  }
}
