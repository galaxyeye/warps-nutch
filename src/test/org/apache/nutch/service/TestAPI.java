...
public class TestAPI {
  ...
  private static String JOB_TYPE = "JOB_TYPE";
  private static String CONF_ID = "CONF_ID";
  ...
  public void testJobsAPI() throws Exception {
    ...
    // create
    Map<String, Object> map = new HashMap<>();
    map.put(JOB_TYPE, JobType.READDB.toString());
    map.put(CONF_ID, "default");
  ...
  public void testStopKill() throws Exception {
    ...
    // create
    Map<String, Object> map = new HashMap<>();
    map.put(JOB_TYPE, JobType.CLASS.toString());
    ...
    ClientResource stopCli = new ClientResource(baseUrl + JobResource.PATH + "?job=" + jobId + "&cmd=stop");
    r = stopCli.get();
    ...
    System.out.println(jobId);
    assertTrue(jobId.startsWith("default-CLASS-"));
    ClientResource killCli = new ClientResource(baseUrl + JobResource.PATH + "?job=" + jobId + "&cmd=abort");
    r = killCli.get();
    assertEquals("true", r.getText());
    killCli.release();
    Thread.sleep(2000); // wait for the job to finish
  ...
  public static class SpinningJob extends NutchJob {
