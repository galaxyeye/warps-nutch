...
public class FetchUtil {
  ...
  static public void setFetchTime(WebPage page) {
    ...
    // StringBuilder fetchTimeHistory = new StringBuilder();
    String fetchTimeHistory = TableUtil.getMetadata(page, Metadata.META_FETCH_TIME_HISTORY);
    if (fetchTimeHistory == null) {
      fetchTimeHistory = String.valueOf(fetchTime);
    }
    else {
      fetchTimeHistory += ",";
      fetchTimeHistory += String.valueOf(fetchTime);
    }
    TableUtil.putMetadata(page, Metadata.META_FETCH_TIME_HISTORY, String.valueOf(fetchTimeHistory));
  }
}
