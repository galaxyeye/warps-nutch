 ...
 * accordingly.
 */
public class DbUpdateJob2 extends NutchJob implements Tool {
  public static final Logger LOG = LoggerFactory.getLogger(DbUpdateJob2.class);
  ...
  public DbUpdateJob2(Configuration conf) {
    setConf(conf);
  }
  ...
  public void update(Path crawlDb, Path[] segments, boolean normalize, boolean filter, boolean additionsAllowed, boolean force)
      throws IOException {
    ...
    JobConf job = createJob(getConf(), crawlDb);
...
//    job.setReducerClass(CrawlDbReducer.class);
  ...
  public static void main(String[] args) throws Exception {
    int res = ToolRunner.run(NutchConfiguration.create(), new DbUpdateJob2(), args);
    System.exit(res);
  }
...
//    boolean filter = getConf().getBoolean(CrawlDbFilter.URL_FILTERING, false);
    ...
    try {
      update(crawlDb, dirs.toArray(new Path[dirs.size()]), normalize, filter, additionsAllowed, force);
